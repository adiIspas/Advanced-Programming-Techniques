	...
	static void acopera(int [][]m, int xa, int ya, int xb, int yb, int lg, int cg){
		...
		if((int)Math.abs(xa - xb) == 1){
			...
			if(lg % 2 == 0 && cg % 2 == 0){
				...
				m[lg + 1][cg] = m[lg + 1][cg + 1] = m[lg][cg + 1] = 3; 
			}
			else
				// pozitia goala in dreapta sus
				if(lg % 2 == 0 && cg % 2 == 1){
				
					m[lg][cg - 1] = m[lg + 1][cg - 1] = m[lg + 1][cg] = 1; 
				}
				else
					// pozitia goala in stanga jos
					if(lg % 2 == 1 && cg % 2 == 0){
						
						m[lg - 1][cg] = m[lg][cg + 1] = m[lg - 1][cg + 1] = 2; 
					}
					else
						// pozitia goala in dreapta jos
						if(lg % 2 == 1 && cg % 2 == 1){
							
							m[lg][cg - 1] = m[lg - 1][cg - 1] =  m[lg - 1][cg] = 4;  
						}
			return;
		}
		...
		if(lg <= xm && cg <= ym){
			...
			m[xm + 1][ym] = m[xm + 1][ym + 1] =  m[xm][ym + 1] = 3;
		...
		// pozitia goala este in subpatratul dreapta sus
			if(lg <= xm && cg >= ym){
				...
				m[xm][ym] = m[xm + 1][ym] = m[xm + 1][ym + 1] = 1;
			...
			if(lg >= xm && cg <= ym){
				...
				m[xm][ym] = m[xm][ym + 1] = m[xm + 1][ym + 1] = 2;
